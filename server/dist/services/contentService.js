...
class ContentService {
     ...
     */
    getPublicItems(entityType, category) {
        ...
        const items = Array.from(store.values()).filter((item) => {
            if (item.status !== 'PUBLISHED')
                return false;
            if (item.scheduledPublishAt && new Date(item.scheduledPublishAt).getTime() > now)
                return false;
            if (item.scheduledArchiveAt && new Date(item.scheduledArchiveAt).getTime() <= now)
                return false;
     ...
     */
    getPublicItemBySlug(entityType, slug) {
        const store = this.getStore(entityType);
        if (!store)
            return null;
        const item = Array.from(store.values()).find((i) => i.slug === slug && i.status === 'PUBLISHED');
        return item || null;
    }
    /**
     * Admin query: Returns all items including DRAFT, IN_REVIEW, ARCHIVED
     */
    getAdminItems(entityType) {
        const store = this.getStore(entityType);
        if (!store)
            return [];
     ...
     */
    createItem(entityType, data, author) {
        const store = this.getStore(entityType);
        if (!store)
     ...
     */
    updateItem(entityType, id, updates, author, changesSummary = 'Content update') {
        const store = this.getStore(entityType);
        if (!store)
            throw new Error(`Invalid entity type: ${entityType}`);
     ...
     */
    setStatus(entityType, id, status, author) {
        return this.updateItem(entityType, id, { status }, author, `Status transitioned to ${status}`);
    }
    /**
     * Restore previous revision
     */
    restoreRevision(entityType, id, revisionId, author) {
        const store = this.getStore(entityType);
        if (!store)
            throw new Error(`Invalid entity type: ${entityType}`);
        const existing = store.get(id);
        if (!existing)
            throw new Error(`Item ${id} not found`);
        const targetRev = existing.revisions.find((r) => r.revisionId === revisionId);
        if (!targetRev)
            throw new Error(`Revision ${revisionId} not found`);
        return this.updateItem(entityType, id, { ...targetRev.snapshot }, author, `Restored revision ${revisionId}`);
    }
    /**
     * Soft-delete / Archive content entity
     */
    archiveItem(entityType, id, author) {
    ...
    updateHomepageConfig(updates, authorEmail) {
        this.homepageConfig = {
            ...this.homepageConfig,
            ...updates,
            updatedAt: new Date().toISOString(),
