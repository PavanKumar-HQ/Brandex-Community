...
router.post('/auth/login', async (req, res) => {
    ...
    const result = await authService.authenticate(email, password, req.ip || req.socket.remoteAddress);
    if (!result) {
...
const getEntityPermission = (entity) => `content:${entity}`;
/**
 * List all items in entity (including drafts & archives)
 */
router.get('/content/:entity', requireAuth, (req, res, next) => {
 ...
 */
router.post('/content/:entity', requireAuth, (req, res, next) => {
    requirePermission(getEntityPermission(req.params.entity))(req, res, next);
}, (req, res) => {
    const { entity } = req.params;
 ...
 */
router.put('/content/:entity/:id', requireAuth, (req, res, next) => {
    requirePermission(getEntityPermission(req.params.entity))(req, res, next);
}, (req, res) => {
    const { entity, id } = req.params;
    const item = contentService.updateItem(entity, id, req.body, {
        id: req.user.sub,
        email: req.user.email,
        role: req.user.role
 ...
 */
router.put('/content/:entity/:id/status', requireAuth, (req, res, next) => {
    requirePermission(getEntityPermission(req.params.entity))(req, res, next);
}, (req, res) => {
    const { entity, id } = req.params;
 ...
 */
router.post('/content/:entity/:id/revisions/:revisionId/restore', requireAuth, (req, res, next) => {
    requirePermission(getEntityPermission(req.params.entity))(req, res, next);
}, (req, res) => {
...
/* =========================================================================
   ...
   ========================================================================= */
router.get('/enquiries', requireAuth, requirePermission('enquiry:read'), (req, res) => {
    const enquiries = crmService.getTemporaryEnquiries();
    res.json({ success: true, count: enquiries.length, data: enquiries });
});
router.put('/enquiries/:refCode/status', requireAuth, requirePermission('enquiry:update'), (req, res) => {
 ...
 */
router.post('/system/cleanup', requireAuth, requireSuperAdmin, (req, res) => {
